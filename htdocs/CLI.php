...
if(!defined('PHP_CGP_CLI_CLASS')) {
    ...
    class CLI {
        ...
        function GetAccountInfo($accountName,$key) {
            ...
            $this->send('GetAccountInfo '.$accountName.' Key '.$key);
            $this->_parseResponse();
            if($this->isSuccess()) {
                return $this->parseWords($this->getWords());
            }
       ...
       function ListDomainObjects($domainName) {
            $command = 'LISTDOMAINOBJECTS';
            if($domainName != '') $command .= ' '.$domainName;
            $this->send($command);
            $this->_parseResponse();
            if($this->isSuccess()) {
        ...
        function UpdateDomainSettings($params) {
            if(!array_key_exists("domainName",$params)) {
                ...
                $command = 'UpdateDomainSettings '.$params["domainName"];
                if(is_array($params["settings"]))
                    $command .= ' '.$this->printWords($params["settings"]);
                $this->send($command);
                $this->_parseResponse();
        ...
        function CreateDomain($domainName,$params='') {
            ...
            $command = 'CreateDomain '.$domainName;
        ...
        function readDictionary() {
            ...
            while($this->span < $this->len) {
                ...
                if(substr($this->data,$this->span,1) == '}') {
                    ...
                    $this->skipSpaces();
                    if(substr($this->data,$this->span,1) != ';') {
                        if (isset($result['mode']) && isset($result['ConfirmationID'])) {
                            return $result['mode'];
                        } else {
        ...
        function GetStrorageFileInfo($accountName)
        {
            $command = 'GetStrorageFileInfo';
            if($accountName == '')
                die('usage: $cli->GetAccountRules(string $accountName)'."\n");
            $this->send('GETSTORAGEFILEINFO '.$accountName);
            $this->_parseResponse();
            if($this->isSuccess()) {
                return $this->parseWords($this->getWords());
            }
